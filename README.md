...
| **Català** (`ca`) — *projecte AINA · UPC* | Banco clínico propio completo (pares mínimos, expansión semántica, Audición, Lenguaje, TEA, Dislexia y Test de Ling): **858 locuciones enumeradas en el corpus**. La voz neuronal **Ona** (Piper `ca_ES`) está configurada en la tubería pero **todavía no sintetizada**: hasta que corra `voice-assets.yml`, la variedad suena con la voz catalana del dispositivo, y la tarjeta de voz lo dice en pantalla. | Sistema `ca-ES` con recaída a `expo-speech`. |
