...
| **Català** (`ca`) — *projecte AINA · BSC* | Banco clínico propio completo (pares mínimos, expansión semántica, Audición, Lenguaje, TEA, Dislexia y Test de Ling): **858 locuciones enumeradas en el corpus**. La voz es **Matxa-TTS** del projecte AINA (`projecte-aina/matxa-tts-cat-multiaccent`, export end-to-end `matxa_multiaccent_wavenext_e2e.onnx`), que **no es Piper**: es Matcha-TTS (flow matching) con vocóder propio y frontend fonémico (espeak-ng `ca`), así que tiene motor propio en la tubería. **Todavía sin sintetizar**: mientras tanto la variedad suena con la voz catalana del dispositivo, y la tarjeta de voz lo dice en pantalla. | Sistema `ca-ES` con recaída a `expo-speech`. |
