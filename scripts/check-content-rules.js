 ...
 *
 * Comprueba sobre los CINCO bancos (es, es-DO, eu, gl, en) las reglas que ACOPROS fijó
...
try {
  const entradas = [
    ...
    path.join(ROOT, 'src', 'valeriaSemanticExpansionEn.ts'),
  ];
  execSync(
  ...
  const en = require(path.join(tmp, 'valeriaSemanticExpansionEn.js'));
  ...
  const BANCOS = [
    ...
    // El inglés pide con «Say:», no con «Di:». Sin esta entrada el banco `en`
    // sencillamente no se revisaría: el gate no falla por un banco que no mira.
    { lang: 'en', scenarios: en.DAILY_SCENARIOS_EN, categories: en.LEXICAL_CATEGORIES_EN, sequences: en.PROGRESSION_SEQUENCES_EN, capsules: en.CONTRAST_CAPSULES_EN, pedir: /\bsay:\s*/i },
  ];
...
console.log('\n✓ Los cinco bancos cumplen las reglas de contenido (ES-06, ES-10, ES-13).');
